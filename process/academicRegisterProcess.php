...
if (empty($fname)) {
   ...
   echo "Last Name Must Be Less Than 50 characters";
    ...
    $r = Database::search(" SELECT * FROM `academic` WHERE `email` = '".$email."' OR `username` = '".$username."' ");
    if ($r->num_rows > 0) {
    ...
    Database::iud("INSERT INTO `s_m_s`.`academic` (`fname`, `lname`, `nic`, `email`, `username`, `password`, `registration_date`) 
    VALUES ('".$fname."', '".$lname."', '".$nic."', '".$email."', '".$username."', '".$password."', '".$date."');
    ");
    echo "success";
    }
}
