    ...
    if ($ucount == 1) {
        ...
        if ($detailsRow["verification_code"] == 0) {
            ...
            if ($detailsRow["verification_status_id"] == "1") {
                ...
                if ($user_type == 1) {
                    $query2 = "UPDATE `s_m_s`.`teacher` SET `verification_status_id` = '1' WHERE (`id` = '" . $userID . "')";
                } else if ($user_type == 2) {
                    $query2 = "UPDATE `s_m_s`.`academic` SET `verification_status_id` = '1' WHERE (`id` = '" . $userID . "')";
                } else if ($user_type == 3) {
                    $query2 = "UPDATE `s_m_s`.`student` SET `verification_status_id` = '1' WHERE (`id` = '" . $userID . "')";
                }
